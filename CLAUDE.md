...
`sw.js` — bump `CACHE_VERSION` on **every deploy**. Currently `fore-v116`.
...
1. Bump `CACHE_VERSION` in `sw.js` on every deploy (currently `fore-v116` → increment to `fore-v117`, etc.)
